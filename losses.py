...
class VGG(torch.nn.Module):
    ...
    def forward(self, pred, true_encoder):
        true_encoder = true_encoder[-1]
        pred_encoder = self.encoder(pred.half())[-1]

        return self.penalty(true_encoder, pred_encoder)
...
class Dice_Loss(torch.nn.Module):
    ...
    def forward(self, pred, true):
        ...
        for k in range(pred.shape[1]):
            ...
            norm = torch.sum(pred * pred) + torch.sum(true * true)

            dice_loss += 1 - 2 * inter / (norm + self.epsilon)
